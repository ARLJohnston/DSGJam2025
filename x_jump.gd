...
func _ready() -> void:
	...
	$XJumpUpgradeLevelStar1.material.set_shader_parameter("shine_enabled", false)   
	$XJumpUpgradeLevelStar2.material.set_shader_parameter("shine_enabled", false) 
	$XJumpUpgradeLevelStar3.material.set_shader_parameter("shine_enabled", false)
	...
	if (upgrade_level < MAX_LEVEL):
		...
		match upgrade_level:
			1:
				...
				$XJumpUpgradeLevelStar1.material.set_shader_parameter("shine_enabled", true) 
			2:
				$XJumpUpgradeLevelStar2.modulate = get_metal_color(MetalColor.SILVER) 
				$XJumpUpgradeLevelStar2.material.set_shader_parameter("shine_enabled", true)
			3:
				$XJumpUpgradeLevelStar3.modulate = get_metal_color(MetalColor.GOLD) 
				$XJumpUpgradeLevelStar3.material.set_shader_parameter("shine_enabled", true)
			_:
				print("Invalid level")
